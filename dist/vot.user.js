  ...
  <path fill="currentColor" d="M7 15q1.25 0 2.125-.875T10 12t-.875-2.125T7 9t-2.125.875T4 12t.875 2.125T7 15m0 3q-2.5 0-4.25-1.75T1 12t1.75-4.25T7 6q2.025 0 3.538 1.15T12.65 10h8.375L23 11.975l-3.5 4L17 14l-2 2l-2-2h-.35q-.625 1.8-2.175 2.9T7 18"/>
	...
	var VOTButton = class {
		...
		createElements() {
			...
			container.append(translateButton, separator, downloadButton, downloadVideoButton, separatorDl, pipButton, separator2, menuButton);
			return {
				container,
				translateButton,
				separator,
	...
	var OverlayView = class OverlayView {
		...
		*/
		...
		initUIEvents() {
			...
			this.bindPrimaryAction(this.votButton.downloadButton, () => {
				closeMenu();
				this.events["click:downloadTranslation"].dispatch();
			}, signal);
			this.bindPrimaryAction(this.votButton.downloadVideoButton, () => {
				closeMenu();
	...
	var UIManager = class {
		...
		*/
		...
		clearDownloadTranslation(videoHandler) {
			...
			if (this.votOverlayView?.votButton) this.votOverlayView.votButton.showDownloadButtons(false);
		}
		...
		stopTranslate() {
			...
			const cleanup = async () => {
				...
				if (overlayView) {
					...
					if (overlayView.votButton) overlayView.votButton.showDownloadButtons(false);
				}
		...
		*/
		afterUpdateTranslation(audioUrl) {
			...
			if (this.videoData && !this.videoData.isStream) {
				...
				if (overlayView.votButton) overlayView.votButton.showDownloadButtons(true);
